...
def categorize_risk(polarity_score):
  if polarity_score >= 0.05:
    return "High"
  else:
    return "Low"

sample["patient_risk"] = sample["polarity_patient_convo"].apply(categorize_risk)
print(sample[["patient_conversation", "polarity_patient_convo", "patient_risk"]])
...
start_time =datetime(2025, 7, 19, 6, 30, 0, tzinfo=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ") #change the time for every time it runs
url = "https://api.cal.com/v2/bookings"

patient_name = input("Enter patient name: ")
patient_polarity = float(input("Enter patient conversation polarity: "))
...
patient_risk = categorize_risk(patient_polarity)

print(f"Patient: {patient_name}")
print(f"Polarity: {patient_polarity}")
print(f"Risk: {patient_risk}")
